...
install-contract-bundle-dev:
	${AGD} tx swingset install-bundle ${CONTRACT_REF_BUNDLE_ID} \
		--from=gov1 --node=$(DEV_RPC)  --chain-id=${DEV_CHAIN_ID} --keyring-backend=test --gas=auto --yes -b block
...
submit-proposal-dev:
	${AGD} tx gov submit-proposal swingset-core-eval startSimpleExchange-permit.json startSimpleExchange.js \
		--title="Enable simpleExchange" --description="Evaluate startSimpleExchange.js" --deposit=1000000ubld \
		--gas=auto --gas-adjustment=$(GAS_ADJUSTMENT) \
		--from=gov1 --node=$(DEV_RPC)  --chain-id=${DEV_CHAIN_ID} --keyring-backend=test --yes -b block
...
submit-core-eval-dev: install-contract-bundle-dev install-manifest-bundle-dev submit-proposal-dev vote-dev